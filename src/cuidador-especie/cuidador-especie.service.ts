...
export class CuidadorEspecieService {
  ...
  async create(createCuidadorEspecieDto: CreateCuidadorEspecieDto): Promise<{ message: string }> {
  ...
  async remove(deleteRestoreCuidadorEspecieDto: DeleteRestoreCuidadorEspecieDto): Promise<{ message: string }> {
    const result = await this.cuidadorEspecieRepository.query(`
  ...
  async restore(restoreCuidadorEspecieDto: DeleteRestoreCuidadorEspecieDto): Promise<{ message: string }> {
    const result = await this.cuidadorEspecieRepository.query(`
              DECLARE @MENSAJE AS NVARCHAR(100);
